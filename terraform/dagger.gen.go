...
import (
	...
	"dagger/terraform/internal/telemetry"
...
type RedhatModule = dagger.RedhatModule

type RedhatPackages = dagger.RedhatPackages
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Terraform":
		...
		case "File":
			...
			return (*Terraform).File(&parent), nil
		case "Directory":
			var parent Terraform
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Terraform).Directory(&parent, prefix), nil
		case "Installation":
			var parent Terraform
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Terraform).Installation(&parent, container), nil
		case "Container":
			var parent Terraform
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Terraform).Container(&parent), nil
		case "":
			var parent Terraform
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Terraform").
					...
					WithField("Platform", dag.TypeDef().WithScalar("Platform")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Terraform")).
							WithArg("version", dag.TypeDef().WithKind(StringKind)).
							WithArg("platform", dag.TypeDef().WithScalar("Platform").WithOptional(true)))), nil
	default:
